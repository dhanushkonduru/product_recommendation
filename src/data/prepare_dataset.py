...
def filter_users_items(df: pd.DataFrame, min_user_interactions: int = 3, 
                       min_item_interactions: int = 5, max_items: int = 1000) -> pd.DataFrame:
    """
    Filter users and items to ensure minimum interactions.
    Limited to top max_items by interaction count to control dataset size.
    ...
    Args:
        ...
        max_items: cap total items to this number (top by popularity)
    ...
    print(f"After item interaction filter ({min_item_interactions}+): {len(df_filtered)} interactions, {df_filtered['item_id'].nunique()} items")
    
    # Limit to top max_items by popularity
    if df_filtered['item_id'].nunique() > max_items:
        top_items = df_filtered['item_id'].value_counts().head(max_items).index
        df_filtered = df_filtered[df_filtered['item_id'].isin(top_items)].copy()
        print(f"After limiting to top {max_items} items: {len(df_filtered)} interactions, {df_filtered['item_id'].nunique()} items")
    ...
    print(f"After user interaction filter ({min_user_interactions}+): {len(df_filtered)} interactions, {df_filtered['user_id'].nunique()} users")
    
    # Re-filter items in case some lost all their users
...
def prepare_metadata(metadata_gen, valid_items: Set[str]) -> Dict[str, Dict]:
    """
    Prepare metadata dictionary indexed by asin.
    Extract: title, brand, imageURLHighRes
    
    Args:
        metadata_gen: Generator/iterator of metadata items (from gzip stream)
        valid_items: Set of item_ids that are in the filtered reviews
        
    Returns:
        Dictionary: {asin: {title, brand, product_text, image_url}}
    """
    metadata_dict = {}
    
    for item in metadata_gen:
        asin = item.get('asin')
        if not asin or asin not in valid_items:
            continue
...
def main():
    ...
    print("Scaling to full Fashion dataset")
    print("="*60)
    
    # Load data
    print("\nLoading reviews data (streaming)...")
    reviews = load_jsonl_gz("reviews_Fashion.json.gz")
    ...
    df_filtered = filter_users_items(df, min_user_interactions=3, min_item_interactions=5, max_items=1000)
    ...
    # Load and prepare metadata (only for filtered items)
    print("\n" + "-"*60)
    print("Loading and preparing metadata (optimized)...")
    print(f"Loading metadata only for {len(all_items)} filtered items...")
    metadata_gen = load_jsonl_gz("meta_Fashion.json.gz")
    metadata_dict = prepare_metadata(metadata_gen, valid_items=all_items)
    ...
    print("\nReady for model training or Streamlit inference")
